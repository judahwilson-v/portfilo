  ...
  const updateCardTransforms = useCallback(() => {
    ...
    cardsRef.current.forEach((card, index) => {
      ...
      if (blurAmount) {
        ...
        for (let compareIndex = 0; compareIndex < cardsRef.current.length; compareIndex += 1) {
          const compareCardTop = getElementOffset(cardsRef.current[compareIndex]);
          const compareTriggerStart =
            compareCardTop - stackPositionPx - itemStackDistance * compareIndex;

          if (scrollTop >= compareTriggerStart) {
            topCardIndex = compareIndex;
          }
        }
